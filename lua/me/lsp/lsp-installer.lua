...
-- for language server list and their configurations
...
lsp_installer.on_server_ready(function(server)
	local opts = {
	 ...
	 if server.name == "pyright" then
	 	local pyright_opts = require("me.lsp.settings.pyright")
	 	opts = vim.tbl_deep_extend("force", pyright_opts, opts)
	 end

	if server.name == "omnisharp" then
		local omnisharp_opts = require("me.lsp.settings.omnisharp")
